/***
 *baidu Admin JavaScript
 *
 *	http://www.baidu.net
 *
 *	Copyright © 2010 baidu.net All Rights Reserved.
 **/
...
$().ready( function() {
	...
	if ($listForm.size() > 0) {
		...
		var $updateButton = $("#updateButton");// 更新按钮
	...
	// 批量修改
		$updateButton.click( function() {
			var url = $(this).attr("url");
			var $idsCheckedCheck = $("#listTable input[name='ids']:checked");
			$.dialog({type: "warn", content: "您确定要操作这些数据吗?", ok: "确 定", cancel: "取 消", modal: true, okCallback: batchDelete});
			function batchDelete() {
				$.ajax({
					url: url,
					data: $idsCheckedCheck.serialize(),
