...
import net.app.FileInfo.FileType;
import net.app.Message;
import net.app.Setting;
import net.app.Setting.AccountLockType;
import net.app.Setting.CaptchaType;
import net.app.Setting.ConsultationAuthority;
import net.app.Setting.ReviewAuthority;
import net.app.Setting.RoundType;
import net.app.Setting.StockAllocationTime;
import net.app.Setting.WatermarkPosition;
import net.app.service.CacheService;
...
public class SettingController extends BaseController {
	...
	public String update(Setting setting, MultipartFile watermarkImageFile, RedirectAttributes redirectAttributes) {
		...
		try {
			org.springframework.core.io.Resource resource = new ClassPathResource(CommonAttributes.WEBAPP_PROPERTIES_PATH);
			...
			if (setting.getIsDevelopmentEnabled()) {
				if (!templateUpdateDelay.equals("0") || !messageCacheSeconds.equals("0")) {
					...
					properties.store(outputStream, "LFX PROPERTIES");
				}
			} else {
				if (templateUpdateDelay.equals("0") || messageCacheSeconds.equals("0")) {
					outputStream = new FileOutputStream(resource.getFile());
