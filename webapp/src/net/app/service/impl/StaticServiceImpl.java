...
import net.app.dao.ArticleDao;
import net.app.dao.BrandDao;
import net.app.dao.ProductDao;
import net.app.dao.PromotionDao;
...
import net.app.util.FreemarkerUtils;
...
public class StaticServiceImpl implements StaticService, ServletContextAware {
	...
	@Transactional(readOnly = true)
	public void buildPhone(Product product) {
		
		Template template = templateService.get("phoneProductContent");
		if (product.getIsMarketable()) {
			Map<String, Object> model = new HashMap<String, Object>();
			model.put("product", product);
			String path="/product/phone/"+product.getId()+".html";
			build(template.getTemplatePath(),path, model);
		}
